...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
 ...
 // ou compononent ou object  : représente une classe "bean"  
    ...
    protected void configure(HttpSecurity http) throws Exception {
        ...
        http.authorizeRequests().antMatchers("/", "/login", "/logout", "/register", "/logoutSuccessful", "/403", "/registerSuccessful").permitAll();
 
        // pages requires login as ROLE_USER or ROLE_ADMIN.
        // If no login, it will redirect to /login page.
        http.authorizeRequests().antMatchers("/userInfo", "/editInfo", "/deleteAccount", "/newSale").access("hasAnyRole('ROLE_USER', 'ROLE_ADMIN')");
